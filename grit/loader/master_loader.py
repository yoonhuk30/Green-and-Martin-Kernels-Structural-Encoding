...
from grit.loader.dataset.ocb import OCBDataset
...
def load_dataset_master(format, name, dataset_dir):
    ...
    # GraphGym default loader for Pytorch Geometric datasets
        ...
        if "ckt" in name:
            dataset = preformat_OCBDataset(dataset_dir, name)
        else:
            raise ValueError(f"Unsupported OCB dataset: {name}")
    ...
    for key, pecfg in cfg.items():
        if key.startswith('posenc_') and pecfg.enable:
            ...
            if pe_name == "RRWP":
                pe_type_list.append(pecfg.type)
...
def preformat_OCBDataset(dataset_dir, name):
    """Load and preformat datasets from OCB Dataset.

    Args:
        dataset_dir: path where to store the cached dataset
        name: name of the specific dataset in the OCB (ckt_bench_101, ckt_bench_301)
    Returns:
        PyG dataset object
    """
    dataset = join_dataset_splits(
        [OCBDataset(root=dataset_dir, name=name, split=split) for split in ["train", "val", "test"]]
    )
